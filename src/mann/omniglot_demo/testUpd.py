...
def omniglot():
    ...
    def update_tensor(V, dim2, val):
        ...
        Z = tf.scan(body, elems=(V, dim2, val), initializer=tf.constant(
            1, shape=V.get_shape().as_list()[1:], dtype=tf.float32), name="Scan_Update")
        return Z

    print 'Compiling the Model'

    tt1 = tf.Variable(initial_value=np.arange(0, 36).reshape(
        (6, 6)), dtype=tf.float32, name='Matrix')
    ix = tf.Variable(initial_value=np.arange(0, 6), name='Indices')
    val = tf.Variable(initial_value=np.arange(100, 106),
                      name='Values', dtype=tf.float32)

    tt = tf.concat_v2([tt1[:3], tf.reshape(
        tf.range(0, 6, dtype=tf.float32), shape=(1, 6)), tt1[3:]], axis=0)
    ...
    writer = tf.summary.FileWriter(
        '/tmp/tensorflow', graph=tf.get_default_graph())
